...
map_class = {0: "person",1: "bicycle",2: "car",3: "motorcycle",4: "bus"}
# ─── TensorRT Logger ─────────────────────────────────────────────────────────
...
def preprocess(image_path: str, input_h: int, input_w: int):
    """
    Resize ตรง input_h×input_w (640×640) → RGB → CHW → float32 → [0,1]
    ไม่ใช้ letterbox เพราะโมเดลเทรนมาด้วยภาพ 640×640 พอดี
    Returns: blob (C,H,W), original image, scale_x, scale_y
    """
    ...
    # scale factor convert box to original size
    scale_x = orig_w / input_w   # Ex 1280/640 = 2.0
    scale_y = orig_h / input_h

    blob = resized[:, :, ::-1].transpose(2, 0, 1).astype(np.float32) / 255.0
    return np.ascontiguousarray(blob), img_bgr, scale_x, scale_y


# ─── Postprocessing ──────────────────────────────────────────────────────────

def postprocess_yolov8(raw, orig_w, orig_h, scale_x, scale_y,
                       num_classes=5, conf_thresh=0.5, iou_thresh=0.45):
    """
    Parse YOLOv8 output shape [1, 9, 8400]
      - dim 1 row 0-3 : cx, cy, w, h  (in 640×640 pixel space)
    ...
    # Convert 640×640 coords → original image coords
...
def main():
    ...
    parser.add_argument("--no-show",     action="store_true",   help="headless —no display")
    ...
    blob, orig_img, scale_x, scale_y = preprocess(
